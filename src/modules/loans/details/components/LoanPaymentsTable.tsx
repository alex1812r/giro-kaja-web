...
type LoanPaymentsTableProps = {
  ...
  currency: string;
  onSelectPayment?: (payment: LoanPayment) => void;
};
        ...
        <>
              ...
              <thead>
                <tr className="border-b border-border bg-surface-muted text-xs text-text-secondary">
                  <th className="px-4 py-3 font-medium">
                    {t("loans.paymentDateColumn")}
                  </th>
                  <th className="px-4 py-3 text-right font-medium">
                    {t("loans.interest")}
                  </th>
                  <th className="px-4 py-3 text-right font-medium">
                    {t("loans.amortizationCapital")}
                  </th>
                  <th className="px-4 py-3 text-right font-medium">
                    {t("loans.totalPaid")}
                  </th>
                  <th className="px-4 py-3 text-center font-medium">
                    {t("loans.statusColumn")}
                  </th>
                </tr>
              </thead>
              <tbody>
          ...
          <div className="border-t border-border bg-surface-muted px-4 py-3">
            <p className="text-xs text-text-secondary">
              {t("loans.showingPayments", { count: payments.length })}
            </p>
          </div>
        </>
      )}
